  ...
  <div class="panel panel-default">
    ...
    <div class="panel-body">
      ...
      <div class="controls-row form-group">
        ...
        <button onclick="customer.getDetails(${customer.id})" type="button" class="btn btn-primary">查询</button>
      ...
      <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
        ...
        <div class="tile lime">
          ...
          <div class="tile-content">
            <div class="number">${customer.point}</div>
            <h3>总积分</h3>
          </div>
        </div>
        <!-- tile end here -->
        ...
        <div class="panel panel-primary">
          ...
          <table id="basic-datatables2" class="table table-bordered table-striped" cellspacing="0" width="100%">
            ...
            <tr>
              ...
              <th class="per10">商品描述</th>
              <th class="per10">数量</th>
              <th class="per10">消耗积分</th>
              <th class="per10">商品图片</th>
            </tr>
            </thead>
            <tbody>
            <c:forEach var="order" items="${mallOrders}">
              <tr>
                <td><fmt:formatDate value="${order.createTime}" pattern="yyyy.MM.dd HH.mm" /> </td>
                <td>${order.commodityName}</td>
                <td>${order.description}</td>
                <td>${order.num}</td>
                <td>${order.num * order.cost_point}</td>
                <td><a href="${order.img}" target="view_window">点击查看</a></td>
              </tr>
            </c:forEach>
            </tbody>
          </table>
    ...
    <div style="padding:15px 30px;">
      <button onclick="customer.getCustomerList(1)" type="button" class="btn btn-primary">返回</button>
...
<script>
  ...
  var data = [
    ...
    { label: "其他:" + (total - wet - dry) + "克",  data: (total - wet - dry)*100/total, color: colours.red}
  ];
