...
<div class="col-lg-12">
  ...
  <div class="panel panel-default">
    ...
    <div class="panel-heading">
      <h3 class="panel-title">全部回收信息</h3>
    </div>
    ...
    <div class="panel-body">
      ...
      <div class="col-lg-12">
        ...
        <div class="panel panel-primary">
          <!-- Start .panel -->
          <div class="panel-heading">
            <h4 class="panel-title">全部回收操作</h4>
          </div>
          <table id="basic-datatables" class="table table-bordered table-striped" cellspacing="0" width="100%">
            <thead>
            <tr>
              <th class="per10">时间</th>
              <th class="per10">回收员</th>
              <th class="per10">垃圾屋</th>
              <th class="per10">操作结果</th>
            </tr>
            </thead>
            <tbody>
            <c:forEach var="recover" items="${allRecoverHistory}">
              <tr>
                <td><fmt:formatDate value="${recover.handleTime}" pattern="yyyy.MM.dd HH.mm" /> </td>
                <td>${recover.name}</td>
                <td>${recover.hname}</td>
                <td><c:choose>
                  <c:when test="${recover.handle_statu == 0}">取消处理</c:when>
                  <c:when test="${recover.handle_statu == 1}">处理中</c:when>
                  <c:when test="${recover.handle_statu == 2}">超时取消</c:when>
                  <c:when test="${recover.handle_statu == 3}">处理成功</c:when>
                </c:choose></td>
              </tr>
            </c:forEach>
            </tbody>
