        ...
        <div class="panel panel-primary">
          ...
          <table id="basic-datatables" class="table table-bordered table-striped" cellspacing="0" width="100%">
            ...
            <tr>
              ...
              <th class="per10">资源</th>
              <th class="per10">价格</th>
              <th class="per10">状态</th>
            </tr>
            </thead>
            <tbody>
            <c:forEach var="order" items="${orderHistoryList}">
              <tr>
                ...
                <td><c:choose><c:when test="${order.status == 1}">支付未完成</c:when>
                  <c:when test="${order.status == 0}">回收成功</c:when></c:choose></td>
              </tr>
            </c:forEach>
            </tbody>
          </table>
